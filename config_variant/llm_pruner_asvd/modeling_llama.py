...
class ASVDLinear(nn.Module):
    def __init__(self, in_features, out_features, rank, train_frac_beta=0.2, bias=False):
        super().__init__()
        ...
        self.ALinear_train = nn.Linear(rank[1], out_features, bias=bias)
...
class LlamaMLP(nn.Module):
    ...
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        layer_name = f"model.layers.{self.layer_idx}.mlp.gate_proj"
        if truncation_ranks.get(layer_name, None) is not None:
            print(f"Using ASVDLinear for {layer_name}")
            self.gate_proj = ASVDLinear(config.hidden_size, self.intermediate_size, truncation_ranks[layer_name])
        else:
        ...
        layer_name = f"model.layers.{self.layer_idx}.mlp.up_proj"
        if truncation_ranks.get(layer_name, None) is not None:
            print(f"Using ASVDLinear for {layer_name}")
            self.up_proj = ASVDLinear(config.hidden_size, self.intermediate_size, truncation_ranks[layer_name])
        else:
        ...
        if truncation_ranks.get(layer_name, None) is not None:
            print(f"Using ASVDLinear for {layer_name}")
            self.down_proj = ASVDLinear(self.intermediate_size, config.hidden_size, truncation_ranks[layer_name])
        else:
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.hidden_size = config.hidden_size
        if self.layer_idx in config.pruned_layers:
            self.num_heads = config.pruned_num_attention_heads
            self.head_dim = config.pruned_hidden_size[0] // self.num_heads
            self.num_key_value_heads = config.pruned_num_key_value_heads
        else:
        ...
        layer_name = f"model.layers.{self.layer_idx}.self_attn.q_proj"
        if truncation_ranks.get(layer_name, None) is not None:
            print(f"Using ASVDLinear for {layer_name}")
            self.q_proj = ASVDLinear(
                self.hidden_size, self.num_heads * self.head_dim, truncation_ranks[layer_name], bias=False
            )
        else:
            self.q_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)

        layer_name = f"model.layers.{self.layer_idx}.self_attn.k_proj"
        if truncation_ranks.get(layer_name, None) is not None:
            print(f"Using ASVDLinear for {layer_name}")
            self.k_proj = ASVDLinear(
                self.hidden_size, self.num_key_value_heads * self.head_dim, truncation_ranks[layer_name], bias=False
            )
        else:
            self.k_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)

        layer_name = f"model.layers.{self.layer_idx}.self_attn.v_proj"
        if truncation_ranks.get(layer_name, None) is not None:
            print(f"Using ASVDLinear for {layer_name}")
            self.v_proj = ASVDLinear(
                self.hidden_size, self.num_key_value_heads * self.head_dim, truncation_ranks[layer_name], bias=False
            )
        else:
