...
def get_calib_data(name, tokenizer, model_id, nsamples, seqlen=256, seed=3):
    ...
    if name == "c4":
        ...
        # )
        traindata = load_dataset("json", data_files={"train": "en/c4-train.00000-of-01024.json.gz"})["train"]
        tot_text = "\n\n".join(traindata["text"])
    elif name == "wikitext2":
        ...
        data_path = "/nvme_data/yuanzhihang/dataset/alpaca_en/alpaca_data_en_52k.json"
        ...
        for _ in range(nsamples):
            while True:
                ...
                if tokenized_sample.shape[1] >= 256 and i not in history:
                    history.append(i)
                    break
            tokenized_samples.append(tokenized_sample[:, :256])
        traindataset = []
        for inp in tokenized_samples:
