...
from decomp_linear import SVDLinear, GradSVDLinear, TrainableDecompLinear
...
def binary_search_truncation_rank(model, sensitivity_dict, calib_loader, args):
    ...
    for layername, param_ratio in tqdm(layers_min_ratio.items()):
        ...
        if param_ratio == default_param_ratio:
            ...
            if args.hadamard:
                svd_linear = HadamardSVDLinear.from_linear(
                    raw_linear,
                    param_ratio=param_ratio,
                    rank_align=args.rank_align,
                )
            elif args.hadamard_both:
                svd_linear = HadamardBothSVDLinear.from_linear(
                    raw_linear,
                    param_ratio=param_ratio,
                    rank_align=args.rank_align,
                )
...
def naive_decomp(model, args):
    ...
    for raw_linear, info in tqdm(linear_info.items()):
        ...
        raw_linear.to("cpu")
        setattr(info["father"], info["name"], svd_linear)
        print(f"decompose {info['full_name']} with ratio {args.param_ratio_target}")
    ed = time.time()
    print(f"decompose time: {ed-st}")
