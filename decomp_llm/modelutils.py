...
@torch.no_grad()
def profile_svdllm_low_resource(model_name, model, calib_loader, dev, add_norm, norm_coef=0.5):
    ...
    for i in tqdm(range(len(layers))):
        ...
        def hook(module, input, output):
            ...
            adds_sum = torch.mean(adds, dim=0)
            # adds_sum = torch.sum(adds, dim=0)
            module.scaling_diag_matrix += adds_sum
            module.scaling_diag_matrix2 += (output[0].abs().mean(dim=-2).detach().view(-1)) ** norm_coef
        ...
        for name in subset:
            ...
            subset[name].scaling_diag_matrix2 = 0
            handles.append(subset[name].register_forward_hook(hook))
        for j in range(inps.shape[0]):
            if "opt" not in model_name:
                # breakpoint()
                if "Qwen" or "Llama-3" in model_name:
                    outs[j] = layer(
                        inps[j].unsqueeze(0),
        ...
        for name in subset:
            ...
            subset[name].scaling_diag_matrix2 = subset[name].scaling_diag_matrix2.cpu()
            if add_norm:
                print(f"{name} max: {subset[name].scaling_diag_matrix2.cpu().max()}")
        torch.cuda.empty_cache()
        for name in subset:
            raw_scaling_diag_matrix = subset[name].scaling_diag_matrix.double().to(dev)
            scaling_diag_matrix2 = subset[name].scaling_diag_matrix2
            try:
                ...
                del eigenvalues
            if add_norm:
                layer_profile[name] = scaling_diag_matrix.cpu(), scaling_diag_matrix2
            else:
