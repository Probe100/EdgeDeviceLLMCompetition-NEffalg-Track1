...
def main(args):
    ...
    if args.block_wise:
        ...
        for i in range(args.iterative_steps):
            ...
            if pruner_type in ['taylor']:
                example_prompts = get_examples(args.calib_dataset, tokenizer, args.num_examples, seq_len = 256).to(args.device)
        ...
        kwargs = {
            ...
            "root_module_types": [Qwen2RMSNorm],
        }
    ...
    if args.save_model:
        ...
        config['pruned_layers']=[i for i in range(args.block_mlp_layer_start, args.block_mlp_layer_end)]
        config['pruned_intermediate_size'] = model.model.layers[args.block_mlp_layer_start].mlp.gate_proj.out_features
        config['pruned_hidden_size'] = model.model.layers[args.block_attention_layer_start].self_attn.q_proj.out_features
        config['pruned_num_attention_heads'] = model.model.layers[args.block_attention_layer_start].self_attn.num_heads
        config['pruned_num_key_value_heads'] = model.model.layers[args.block_attention_layer_start].self_attn.num_key_value_heads
        config["auto_map"] = {
        ...
        os.system(
            "cp ./LLMPruner/models/hf_qwen2_a0m4/configuration_qwen2.py ./LLMPruner/models/hf_qwen2_a0m4/modeling_qwen2.py ./"
            + args.save_path
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--base_model', type=str, default="/mnt/resource/public_models/Meta-Llama-3.1-8B-Instruct", help='base model name')
    ...
    parser.add_argument("--save_path", type=str, default="pruned_qwen2", help='path to save the model')
